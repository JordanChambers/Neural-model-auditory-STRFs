...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *meanrate, *varrate, *psth, *sptime;
	...
	if (nlhs != 4)  
	{
		mexErrMsgTxt("model_Synapse requires 4 output argument.");
	};
    ...
    outsize[1] = (long) ceil(totalstim*tdres*nrep/0.00075);
    ...
    psth	  = mxGetPr(plhs[2]);
    sptime = mxGetPr(plhs[3]);
			
	/* run the model */

	mexPrintf("ANmodel: Zilany, Bruce, Ibrahim, and Carney : Auditory Nerve Model\n");

	SingleAN(px,cf,nrep,tdres,totalstim,fibertype,noiseType,implnt,meanrate,varrate,psth,sptime);

 mxFree(px);

}

void SingleAN(double *px, double cf, int nrep, double tdres, int totalstim, double fibertype, double noiseType, double implnt, double *meanrate, double *varrate, double *psth, double *sptime)
{	
